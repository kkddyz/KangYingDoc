...
SELECT COUNT( IFNULL(english,0) ) AS "英语考生" FROM student_score;	-- 统计null(null变成0了)
SELECT COUNT(english) AS "英语考生数" FROM student_score; -- 不统计null

# 配合条件查询使用 -- 只会统计满足条件的数据 
# 统计男性数量
select count(name) as "男性数量" from student_score where sex = "男";

# 但是注意使用聚合函数后查询到的记录只能为1条
...
先根据分组信息将表中记录分组，然后再使用聚合函数计算分组数据。
...
where在分组之前进行筛选，having则是得到分组记录后筛选分组记录(如果不满足(分组)聚合属性就会被去掉)
...
```mysql
select * from table_name where id > #max_id# order by id limit n;
```
...
```mysql
 select * from table_name where id > #max_id# order by id limit 20, 10;
```

从第三页开始，查询30条记录，去掉前20条
...
```mysql
select * from table_name as a inner join (select id from table_name order by id limit m, n) as b on a.id = b.id order by a.id;
```

该查询同方式1一样，m的值可能很大，但由于内部的子查询只扫描了字段id，而不是整张表，所以性能要强于方式1查询，并且该查询能够解决方式2和方式3不能解决的问题。

> 先通过子查询，将符合limit条件的记录查询出来，再通过内连接
...
INSERT INTO student_info(NAME) VALUES("张山");         # dose not has a default value 
...
# 传入null,会触发非空约束。不传值，会覆盖为默认值AUTO_INCREMENT
# 这样ID default value = auto_increasement 
...
> 主键处理可以通过PRIMARY KEY 直接加在属性上，也可以在语句的末尾指定
>
> ```sql
> CREATE TABLE roles(
...
员工与部门属于一对多的关系；这种方式会导致数据冗余和难以维护的问题；
...
<img src="https://kkddyz-oss-image-hosting-service.oss-cn-hangzhou.aliyuncs.com/image/20220930094911.png" alt="image-20220930094911595" style="zoom:67%;" align="left"/>
